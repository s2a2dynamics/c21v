    ...
    let html = `
      ...
      <body>
        ...
        <div class="grid">
          ${propiedades.map(p => `
            <div class="card">
              <div class="carousel">${(p.galeria || []).map((img:string) => `<img src="${img}" loading="lazy">`).join('')}</div>
              <div class="content">
...
export const chatWithGemini = async (req: Request, res: Response) => {
  try {
    ...
    const result = await aiModel.generateContent("Actúa como un agente inmobiliario experto de Century 21 Venezuela. Sé breve, amable y profesional. El cliente dice: " + userMsg);
