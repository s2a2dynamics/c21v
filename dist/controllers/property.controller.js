...
const parseCache = (jsonRaw) => {
    if (!jsonRaw)
        return null;
    if (typeof jsonRaw === 'object')
        return jsonRaw; // Ya es objeto
    try {
...
const getPropertiesHTML = async (req, res) => {
    try {
        ...
        const { propiedades, fotos } = await service.findAll(req.query.ciudad, req.query.operacion);
        // 2. PROCESAMIENTO Y MAPEO DE DATOS
        const listaFinal = propiedades.map((p, index) => {
            var _a;
            // A. FOTOS: Cruzamos usando 'id_propiedad_real' para evitar conflictos
            const susFotos = fotos
                .filter((f) => f.idPropiedades == p.id_propiedad_real)
                .map((f) => {
                var _a, _b;
            ...
            if (p.cache_asesor) {
                ...
                if ((_a = data === null || data === void 0 ? void 0 : data.asesorThumbnail) === null || _a === void 0 ? void 0 : _a.path)
                    avatarUrl = data.asesorThumbnail.path;
            }
            if (!avatarUrl) {
            ...
            <div class="card">
              <div class="carousel-wrapper">
                <div class="carousel" id="carousel-${p.uniqueId}" onscroll="updateCounter(${p.uniqueId})">
                   ${p.galeria.map((img) => `<img src="${img}" loading="lazy">`).join('')}
                </div>
...
const chatWithGemini = async (req, res) => {
    try {
        ...
        const inventario = propiedades.map((casa) => {
            // Logica fotos para la IA
            const susFotos = fotos
                .filter((f) => f.idPropiedades == casa.id_propiedad_real)
                .map((f) => {
                var _a;
                const data = parseCache(f.cache);
                if ((_a = data === null || data === void 0 ? void 0 : data.propiedadLarge) === null || _a === void 0 ? void 0 : _a.path)
    ...
    4. Solo envía JSON crudo después del separador.`;
        const result = await ai_1.aiModel.generateContent(prompt);
