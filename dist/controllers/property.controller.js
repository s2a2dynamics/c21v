      ...
      <body>
        ...
        <div class="grid">
          ${propiedades.length ? propiedades.map((p) => `
            <div class="card">
              <div class="carousel">${p.galeria.map((src) => `<img src="${src}">`).join('')}</div>
              <div class="content">
...
const chatWithGemini = async (req, res) => {
    ...
    `;
        const result = await ai_1.aiModel.generateContent(toolsPrompt);
        ...
        if (jsonMatch) {
            try {
                ...
                if (params.action === "SEARCH_DB") {
                    ...
                    // Resumen para Gemini
                    const dataSummary = results.slice(0, 4).map(p => `- ${p.encabezado} (${p.municipio}) Precio: ${p.moneda} ${p.precioVenta || p.precioRenta}. LinkZona: ${p.link_zona}`).join('\n');
           ...
           `;
                    const finalRes = await ai_1.aiModel.generateContent(finalPrompt);
